...
namespace Alpha._0.Classes
{
    ...
    public class SimpleTcpClient : IDisposable
    {
        ...
        public bool Connect(string hostNameOrIpAddress, int port)
        {
            ...
            IAsyncResult ar = _client.BeginConnect(hostNameOrIpAddress, port, null, null);
        ...
        {
            if (data == null) { return true; } //数据空也是return true
            var byteData = StringEncoder.GetBytes(data).ToList();
            byteData.Add(Delimiter);
            return Write(byteData.ToArray());
        }
        ...
        public Message WriteLineAndGetReply(string data, TimeSpan timeout)
        {
            lock (_messageSendLock)
            {
                ...
                bool res = WriteLine(data); //如果socket断开 需要处理
                if (!res)
                {
                    return null;
                }
